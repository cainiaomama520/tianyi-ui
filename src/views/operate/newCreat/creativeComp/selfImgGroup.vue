          ...
          <div class="labelItem">
              ...
              <div @click='uploadClick(0)'>
                <el-upload
                  ...
                  <img v-if="currentModel.url[0]" :src="currentModel.url[0]" class="avatar">
                  <i v-else style="color:#598fe6" class="el-icon-upload avatar-uploader-icon">上传图片</i>
                  <div class="progressDiv" v-if="imgFlag && currentIndex ==0">
                    <el-progress class="progressC" style="top:2%;left:11%;" type="circle" color="green" :percentage="videoUploadPercent"></el-progress>
                  </div>
                </el-upload>
              </div>
              <div @click='uploadClick(1)'>
                <el-upload
                  class="avatar-uploader"
                  ...
                  :before-upload="beforeAvatarUpload"
                  :on-progress="uploadVideoProcess">
                  <img v-if="currentModel.url[1]" :src="currentModel.url[1]" class="avatar">
                  <i v-else style="color:#598fe6" class="el-icon-upload avatar-uploader-icon">上传图片</i>
                  <div class="progressDiv" v-if="imgFlag && currentIndex ==1">
                    <el-progress class="progressC" style="top:2%;left:77%" type="circle" color="green" :percentage="videoUploadPercent"></el-progress>
                  </div>
                </el-upload>
              </div>
              <div @click='uploadClick(2)' style="display:inline-block">
                <el-upload
                  class="avatar-uploader"
                  ...
                  :before-upload="beforeAvatarUpload"
                  :on-progress="uploadVideoProcess">
                  <img v-if="currentModel.url[2]" :src="currentModel.url[2]" class="avatar">
                  <i v-else style="color:#598fe6" class="el-icon-upload avatar-uploader-icon">上传图片</i>
                  <div class="progressDiv" v-if="imgFlag && currentIndex ==2">
                    <el-progress class="progressC" style="top:2%;left:142%" type="circle" color="green" :percentage="videoUploadPercent"></el-progress>
                  </div>
                </el-upload>
              </div>
              <div class="tipDiv" style="width:100%;display:inline-block">
                宽高比1.52,大小≤1.5M,456*300≤尺寸≤1368*900,支持JPG、PNG等图片格式
              </div>
						</div>
          </div>
          <creative-title
...
export default {
  ...
  watch: {
    ...
    //     console.log('---',this.currentModel.wordIds);
  ...
  methods: {
    ...
    handleAvatarSuccess(res, file) {
      ...
      if (res.msg == 'success') {
        // this.currentModel.url[0] = res.data.url
        this.$set(this.currentModel.url,0,res.data.url)
        this.currentModel.id[0] = res.data.id
    ...
    handleAvatarSuccess1(res, file) {
      ...
      this.videoUploadPercent = 0;
      if (res.msg == 'success') {
        // this.currentModel.url[1] = res.data.url
        this.$set(this.currentModel.url,1,res.data.url)
        this.currentModel.id[1] = res.data.id
        this.$message.success('图片上传成功')
      }
      else {
        this.$message.error('图片上传失败')
    ...
    handleAvatarSuccess2(res, file) {
      ...
      this.videoUploadPercent = 0;
      if (res.msg == 'success') {
        // this.currentModel.url[2] = res.data.url
        this.$set(this.currentModel.url,2,res.data.url)
        this.currentModel.id[2] = res.data.id
        this.$message.success('图片上传成功')
      }
      else {
        this.$message.error('图片上传失败')
