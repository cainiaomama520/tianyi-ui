  ...
  <div class="data_box" >
    ...
    <div class="data_main" :style="leftBarstatus?'margin-left: 210px;':'margin-left:0'">
        ...
        <el-col :span="24">
          ...
          <el-form ref="form" :model="form" label-width :inline="true" class="demo-form-inline">
            ...
            <el-form-item label="投放">
              <el-select v-model="form.pricing" placeholder="目的" @change="getAdgroupData" size="medium">
                <el-option
            ...
            <el-form-item label="广告计划">
              ...
              <template v-if="isAll=='2'">已选择{{form.selectidList.length}}广告计划</template>
            </el-form-item>
          </el-form>
        </el-col>
      </el-row>
      ...
      <el-row class="table-top">
        <el-col>
          <el-button @click="showLabels=!showLabels">自定义列
            <i class="el-icon-arrow-down"></i>
          </el-button>
          <el-button @click="download">下载报表</el-button>
          <!-- <el-button>分时段下载报表</el-button> -->
          <table-option v-show="showLabels" @changeTableHeader="changeTableHeader" pageType="2"></table-option>
        </el-col>
      </el-row>
      <el-row>
        <el-col>
...
import { getPlanData, getPlanList, initModel, selects,adplan } from 'api/dataReport'
export default {
  data () {
    return {
      ...
      compareTableData: [], 
      groupLabels: [],
      tableLabel: initModel('staticLabel').concat(initModel('staticPlan')),
      seriesData: [],
      pager: {
        currentPage: 1,
  ...
  created: function () {
    this.groupLabels = initModel('data').concat(initModel('idea')).concat(initModel('plan'));
    this.chart_value2 = 'show';
    this.chart_value1 = 'show';
  },
  mounted: function () {
  ...
  methods: {
    ...
    getAdgroupData: function () {
      ...
      getPlanData(chartSearch).then(res => {
    ...
    changeTableHeader: function (arr) {
      this.tableLabel = initModel('staticLabel').concat(initModel('staticPlan')).concat(arr);
    },
	...
	download(){
		...
		let url=this.serverUrl+'/biz/export/ad?adverId='+this.adverId+'&startDate='+this.time1[0]+'&endDate='+this.time1[1]+'&selectidList='+this.form.selectidList.join(",")+'&summarizeType='+this.form.summarizeType+'&landingType='+this.form.landing_type+'&token='+this.token
	
		adplan(chartSearch,{responseType: 'arraybuffer'}).then(res=>{
		 if(res){
