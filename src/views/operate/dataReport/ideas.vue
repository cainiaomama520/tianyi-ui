    ...
    <div class="data_main" :style="leftBarstatus?'margin-left: 210px;':'margin-left:0'">
        ...
        <el-col :span="24">
          ...
          <el-form ref="form" :model="form" label-width :inline="true" class="demo-form-inline">
            ...
            <el-form-item label="投放位置">
              <el-select v-model="form.inventory_type" placeholder="投放位置" @change="getAdgroupData" size="medium">
                <el-option
                  v-for="item in inventory_types"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                ></el-option>
            ...
            <el-form-item label="展现样式">
              <el-select v-model="form.image_mode" placeholder="展现样式" @change="getAdgroupData" size="medium">
                <el-option
                  v-for="item in image_modes"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                ></el-option>
            ...
            <el-form-item label="创意生成方式">
              <el-select
                v-model="form.creative_material_mode"
                placeholder="创意生成方式"
                @change="getAdgroupData"
				size="medium"
              >
                <el-option
                  v-for="item in creative_material_modes"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                ></el-option>
      ...
      <el-row class="table-top">
        <el-col>
          ...
          <table-option v-show="showLabels" @changeTableHeader="changeTableHeader" pageType="3"></table-option>
        </el-col>
      </el-row>
      <el-row>
        <el-col>
            ...
            >
              <template slot-scope="scope">
                ...
                <template v-else>
                  ...
                  <template v-else>
                    <table-cell :item="scope.row" :renderd="item.formatter" :col="item.id"></table-cell>
                    <!-- <p>{{scope.row[item.id]}}</p>-->
                    <p v-show="compare">{{getCompareRowData(scope.$index,item.id)}}</p>
                  </template>
                </template>
...
import "../../../components/ds-table/tableCell.js";
import {
  getIdeasData,
  getPlanList,
  initModel,
...
export default {
  data() {
    return {
      ...
      inventory_types: selects.inventory_types,
      image_modes: selects.image_modes,
      creative_material_modes: selects.creative_material_modes,
      form: {
        compare: false,
        summarizeType: 2,
        inventory_type: 0 /*投放位置*/,
        image_mode: 0 /*展现样式*/,
        creative_material_mode: 0 /*创意生成方式*/,
        selectidList: []
      },
      chart_value2: "",
      chart_value1: "",
      ...
      compareTableData: [],
      groupLabels: [],
      tableLabel: initModel("staticLabel").concat(initModel("staticIdea")),
      seriesData: [],
      pager: {
        currentPage: 1,
  ...
  created: function() {
    this.groupLabels = initModel("data").concat(initModel("idea"));
    this.chart_value2 = "show";
    this.chart_value1 = "show";
  },
  mounted: function() {
  ...
  computed: {
    legendData: function() {
      ...
      if (this.compare) {
        return [
          val1,
          this.time1[0] + "到" + this.time1[1],
          this.time2[0] + "到" + this.time2[1]
        ];
  ...
  methods: {
    getCompareRowData: function(index, value) {
      if (this.compare) {
        if (
          (this.compareTableData.length =
            0 || this.compareTableData[index] == undefined)
        ) {
          return "";
        }
    ...
    getAdgroupList: function() {
      getPlanList({ adverId: this.adverId }).then(res => {
        this.adgroups = res;
      });
    },
    getAdgroupData: function() {
      let chartSearch = {
        ...
        selectidList: this.form.selectidList /*选择业务的ID列表*/,
        inventory_type: this.form.inventory_type /*投放位置*/,
        image_mode: this.form.image_mode,
        creative_material_mode: this.form
          .creative_material_mode /*创意生成方式*/,
    ...
    setNoCampareData: function() {
      /*柱图*/
      let noCampareSeriesData = [];
      let arr = this.tempNoChartData;
      for (var i = 0; i < arr.length; i++) {
        // if (
        //   arr[i].name == this.chart_value1 ||
        //   arr[i].name == this.chart_value2
        // ) {
    ...
    setChartData: function() {
      ...
      if (this.compare) {
        ...
        this.chartData.seriesData = [
          noCampareSeriesData[0],
          noCampareSeriesData[1]
        ];
      }
    },
    changeTime: function(data) {
      ...
      this.time1 = [
        moment(data.time[0]).format("YYYY-MM-DD"),
        moment(data.time[1]).format("YYYY-MM-DD")
      ];
      this.time2 = [
        moment(data.comTime[0]).format("YYYY-MM-DD"),
        moment(data.comTime[1]).format("YYYY-MM-DD")
      ];
      this.getAdgroupData();
    },
    ...
    changeTableHeader: function(arr) {
      this.tableLabel = initModel("staticLabel")
        .concat(initModel("staticIdea"))
        .concat(arr);
    },
    ...
    download() {
      ...
      let url =
        ...
        this.time1[1] +
        "&selectidList=" +
        this.form.selectidList.join(",") +
        "&summarizeType=" +
        this.form.summarizeType +
        "&landingType=" +
        this.form.landing_type +
        "&token=" +
        this.token;

      creative(chartSearch, { responseType: "arraybuffer" }).then(res => {
        if (res) {
